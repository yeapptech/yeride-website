...
seven steps it showed** — both action versions, the Node version, four step
names, the `NPM_TOKEN` the private registry needs — and omitted the fee-label
check entirely. The `.env` step was the one that bit: a single `echo` with `>`
under a comment saying there are six lines, so a reader who followed the comment
literally ended up with a `.env` holding only the last variable and then hit the
failure the comment was warning them about. Nothing asserted the sample, which
is why it drifted; the real step **is** asserted on every pull request
(`scripts/check-deploy-env.mjs`, #90) and is one click away, so the copy bought
...
- **Create env file** — one `echo` per secret into `.env`, the first redirecting
  ...
  environment. Its shape is checked on every pull request (#90): the six names
  must agree with `scripts/env-required.mjs` and `.env.example`, each must come
  from its own same-named secret, and a second `>` — which would truncate the
  file back to one variable — fails.
