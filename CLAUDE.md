...
npm run checks     # Route parity + copy gate + env-example parity (all fast, no deps)
...
`npm run build` runs six things in order, and any one of them fails the build:
